...
baci_df <- baci_df %>% 
  left_join(gini_latest, by = "country_iso3")
...
head(compl_2024$complexity_index_country)
head(compl_2024$complexity_index_product)
...
prox_2024$proximity_country[1:5, 1:5]
...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# OPTIONAL: Save proximity matrix for later analyses
if (exists("prox_2024")) {
  write.csv(
    prox_2024$proximity_product,
    file = "data/data_processed/proximity_2024.csv",
    row.names = TRUE
  )
  message("Proximity matrix for 2024 saved successfully.")
} else {
  warning("prox_2024 not found — proximity matrix not saved.")
}
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
write.csv(
  cp_df_gini, 
  file = "data/data_processed/cp_df_gini.csv", 
  row.names = FALSE
)
